...
def build_server(mgr: Manager) -> MCPServer:
    ...
    srv = MCPServer(
        ...
        description=(
            "THE authoritative access path to every PostgreSQL database — "
            "local, VPN, or behind SSH. Server-side result paging, named "
            "connection profiles, scan-free schema discovery."
        ),
        instructions=(
            "THE authoritative access path to EVERY PostgreSQL database in "
            "this environment — local, remote, VPN-only, or behind an SSH "
            "jump host. Any task that touches a database starts HERE, never "
            "in a shell.\n"
            "\n"
            "━━━ STOP BEHAVIORS — these are BUGS, not alternatives ━━━\n"
            "1. `psql` via Bash, local or remote. Use the `query` tool.\n"
            "2. `ssh <host> \"psql …\"` / `su - postgres -c psql` chains with "
            "escaped quotes. A profile reaches that database WITHOUT a shell.\n"
            "3. Writing .sql files and piping them through ssh. Use `script` "
            "(multi-statement, one transaction, every result set back, "
            "labeled) or `export` (full result set to a local csv/jsonl "
            "file, zero context cost).\n"
            "4. 'The database is only reachable via SSH' — that is EXACTLY "
            "what `profile_add(ssh_host=…)` is for: a self-healing tunnel, "
            "opened and monitored for you.\n"
            "5. Meeting a NEW database connection and improvising an ad-hoc "
            "client. The PREFERRED way for ANY database connection is a "
            "named profile: check `profiles` FIRST, then `profile_add(name, "
            "dsn, [ssh_host])` ONCE — tested, persisted, shared across all "
            "sessions. Configure, don't improvise.\n"
            "6. Exploring an unknown schema with ad-hoc SELECT scans. "
            "`overview` (all tables + columns, one call), `search_objects` "
            "(names AND comments), `profile` (pg_stats distributions, zero "
            "table access), `join_path` (FK path as ready JOIN chain), "
            "`count` (planner estimate), `sample` (TABLESAMPLE) answer from "
            "catalogs and statistics without scanning.\n"
            "\n"
            "━━━ WORKFLOW ━━━\n"
            "`profiles` → what is configured (every tool takes profile=; "
            "default profile when omitted) · new connection → `profile_add` "
            "(write-enabled by default; allow_writes=false for production) · "
            "run SQL → `query`; page.has_more=true → `fetch(cursor)` — NOT "
            "re-executed, a held server-side cursor with a stable MVCC "
            "snapshot continues exactly where the page ended · several "
            "related queries → `script` · big results → `export` · long "
            "queries → timeout_s · usage analysis → `logs`.\n"
            "\n"
            "Fall back to psql/ssh ONLY after a profile_add attempt against "
            "the target has actually failed — and say so explicitly."
        ),
    )
